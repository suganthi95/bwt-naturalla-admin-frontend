...
function AppTimezone() {
  ...
  const query = queryClient.getQueryData(["validateUser"]) as AxiosResponse<{
    data: ValidateUserType;
  }>;

  const [timezoneState, setTimezoneState] = useState(() =>
    query.data.data.timezone === null
      ? Intl.DateTimeFormat().resolvedOptions().timeZone
      : query.data.data.timezone
  );
    ...
    <div className="flex flex-row items-center justify-between rounded-lg border border-slate-200 dark:border-slate-800 p-4 mt-3">
      <div className="space-y-0.5">
        <p className="font-medium">
          <Trans i18nKey={"time_zone"} />
        </p>
        <p className="text-sm text-slate-400">
          <Trans i18nKey={"timezone_description"} />
        </p>
      </div>
      <div>
        <Select
          value={timezoneState}
          onValueChange={(val) => setTimezoneFunction(val)}
        >
