...
function BannerForm({ type, defaultData }: Props) {
    ...
    const { mutate, isPending } = useMutation({
        ...
        onSuccess: () => {
            ...
            toast.success("Request Success", { description: "Banner Image added successfully" });
            setOpen(false);
            reset();
        },
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
                <DialogHeader>
                <DialogTitle>{type === "add" ? "Add New Banner" : "Edit Banner"}</DialogTitle>
                    <form>
                            ...
                            {watch("bannerImage") !== null ? (
                                <div className="relative my-4">
                                    ...
                                    {(watch("bannerImage") as any) instanceof FileList && (
                                        <img
                                        className="h-full w-full object-cover"
                                        src={URL.createObjectURL(watch("bannerImage" as any)[0])}
                                        alt="bannerImage"
                                        />
                                    )}
                                    </div>
                        ...
                        <div className="flex items-center gap-3">
                            <Switch checked={publish} onCheckedChange={setPublish} id="published" /> <Label className="cursor-pointer text-slate-500" htmlFor="published">Publish</Label>
                        </div>
                    </form>
                </DialogHeader>
                <DialogFooter>
