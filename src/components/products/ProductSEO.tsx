...
function ProductSEO() {
  ...
  const { data: productSEODefaults } = useQuery({
    ...
    queryFn: () => getProductSEO(auth?.token ?? "", productId),
    retry: 3,
    refetchOnWindowFocus: false,
    select: (data): ProductSEOFormValues => {
      const { meta_description, meta_image_url, meta_keywords, meta_title } =
        data?.data?.data;
      return {
    ...
    >
      ...
      <div>
        <Label htmlFor="metaKeywords">
          Meta Keywords (Type the keywords and Press "Enter")
        </Label>
        <Input
          id="metaKeywords"
      ...
      <div>
        ...
        {watch("metaImage") !== null ? (
          <div className="relative w-fit my-4">
            <Button
              onClick={() => setValue("metaImage", null)}
              type="button"
              variant="destructive"
            ...
            <div className="rounded-lg w-[250px] h-[250px] overflow-hidden border">
              ...
              {watch("metaImage") instanceof FileList && (
                <img
                  className="h-full w-full object-cover"
                  src={URL.createObjectURL(watch("metaImage")[0])}
                  alt="metaImage"
                />
              )}
            </div>
      ...
      <div className="flex items-center justify-end gap-5 mt-6">
        <Button
          onClick={() => navigate("/products/add/discounts")}
          disabled={isPending}
          type="button"
          variant="secondary"
        >
