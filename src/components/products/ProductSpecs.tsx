...
function ProductSpecs() {
  ...
  useEffect(() => {
    if (productSpecDefaults && productId) {
      ...
      setKeywords(
        Array.isArray(productSpecDefaults?.benefitKeywords)
          ? productSpecDefaults?.benefitKeywords
          : []
      );
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {watch("specificationPDF")?.pdf_url ? (
          <div className="flex flex-row items-center justify-between gap-5">
            <Button
              onClick={() => downloadPDF(watch("specificationPDF")?.pdf_url)}
              className="w-full flex flex-row items-center justify-between"
              type="button"
              variant={"secondary"}
            >
              specifications.pdf <Download className="h-4 w-4" />
            </Button>
            <Button
              onClick={() => setValue("specificationPDF", null)}
              size={"icon"}
              type="button"
              variant={"secondary"}
            >
      ...
      <div>
        <Label>
          Benefit Keywords (Type the benefit keywords and Press "Enter")
        </Label>
        <Input
        ...
        />
        {errors?.benefitKeywords && (
          <p className="text-sm text-red-500 mt-1">
            {errors?.benefitKeywords?.message}
          </p>
        )}
      ...
      <div className="flex items-center justify-end gap-5 mt-6">
        <Button
          onClick={() => navigate("/products/add/product-price")}
          disabled={isPending}
          type="button"
          variant="secondary"
        >
